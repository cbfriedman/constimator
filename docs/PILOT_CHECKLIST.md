...
- Entering the official bid form's line items by hand (`Bid Reconciliation` → the bid form table)
...
- Reports: exporting the estimate + reconciliation summary
...
- "Import from Bid Schedule" and the Markup/Filter dropdowns on the
  Estimate Workspace toolbar — visible but not wired up yet
- Human Review workflow

If a contractor gets curious and tries the AI upload path anyway, that's
...
5. **Enter the official bid form's line items** under `Bid
   ...
   section, one row per bid item. This is manual entry; there's no
   upload-and-auto-fill for this yet.
