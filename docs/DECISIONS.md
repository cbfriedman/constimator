...
**How it works today:** every org gets a 30-day free trial with no Stripe involvement at all (computed from `org.createdAt`, not a stored flag — the length is `TRIAL_DAYS` in [lib/billing.ts](../lib/billing.ts), which is the single source of truth; this document said 14 days for a while after that constant was raised, so check the code rather than this line if the two ever disagree again). Subscribing creates a Stripe Checkout session with quantity = the org's current user count, read live at checkout time. Step 32 added the team-invite flow (`app/team/`) that actually grows headcount past the founding user.

**Seat resync (gap now closed).** For a long time the checkout quantity was the only place seats were ever counted, so an org that subscribed at 2 seats and then grew to 6 kept paying for 2. [lib/seat-sync.ts](../lib/seat-sync.ts) now reconciles the Stripe subscription quantity against the org's real user count, prorated on the next invoice. It runs on `/billing` and on `/team` load rather than at the moment a teammate joins, because joining happens inside migration 0008's `handle_new_user()` Postgres trigger — the app runs no code at that instant, so there is nothing to hook. Reconciling on read is idempotent and self-healing; a missed sync corrects itself the next time either page is opened.

**How to apply:** Don't add a second (e.g. usage-based) billing dimension without revisiting this decision explicitly — the spend cap already covers the cost-containment need usage billing would otherwise be reached for. Note that the spend cap's own accuracy depends on [lib/ai-limits.ts](../lib/ai-limits.ts)'s per-model price table (mirrored in `worker/src/ai-limits.ts`): changing `TAKEOFF_MODEL` or `QUOTE_MODEL` to a model missing from that table prices it at the highest known rate deliberately, so the cap stays protective — but add the real rate rather than leaving it to the fallback.

## Sheet-by-sheet reconciliation

**Decision: the matrix's grain is (plan sheet × bid item), and the column it adds is the quantity *printed* on the sheet — not a per-sheet takeoff.** `/reconciliation/sheets` shows, for every sheet that states a quantity, what that sheet says beside the official bid form quantity, the AI takeoff's measured total, and the estimator's own number. It's built on a second extraction (`worker/src/extract-plan-callouts.ts`) that transcribes schedules, summary tables and notes sheet by sheet, stored in `plan_callout`.

**Why:**
- The existing reconciliation says *that* the estimate and the bid form disagree. It can't say *where the plans disagree with the bid form*, and that's the thing a contractor can RFI: "Sheet C-301's drainage schedule totals 655 LF; Bid Item 8 says 640." A measured quantity can't be cited that way — the agency didn't print it — so the per-sheet number had to be a transcription, consistent with the [[Product wedge]] preference for reading printed values over measuring.
- The AI takeoff stays one total per item, deliberately. Quantities are measured across plan + profile + detail; splitting them per sheet would make the number less accurate to fit the grid. It's shown as a whole-set column on the item, tagged with the sheets it drew from.
- The estimator's column *is* the estimate line's quantity (created at $0 if the item has none), not a third number kept beside it. Two hand-entered quantities for one item would be a question the product can't answer ("which one do I price?").
- Callouts link to bid items by the extractor's own item-number hint when the bid form was imported first, else by a deterministic vocabulary-aware text matcher (`lib/plan-callout-match.ts`) that prefers *not* matching. An unlinked row is a one-click fix and often the most valuable row on the page (scope on the plans that the bid form omits); a wrongly linked row is a discrepancy against the wrong item.

**How to apply:** Specs-stated quantities are not read yet — measurement-and-payment sections describe how, not how many, so the yield is low; extend `extract-plan-callouts.ts` rather than adding a third pass if that changes. Don't re-sync `plan_callout` rows from the job's jsonb once they exist: `bid_id` and `dismissed` are human decisions. The page limit is still `worker/src/rasterize.ts`'s `MAX_PAGES` (20) — a 150-sheet set needs that raised, and the callout pass is what makes raising it affordable to reason about, since it batches.
